class Education::Management::TrainingsController <
  ...
  def create
    ...
    if @training.save
      ...
      redirect_to education_management_trainings_path
    else
  ...
  def edit
    respond_to do |format|
      format.html
      format.js do
        render partial: "education/management/trainings/form",
          locals: {training: @training, button_text: t(".update_project"),
          url: education_management_training_path},
          layout: false
      end
    end
  end
  ...
  def update
    if @training.update_attributes training_params
      ...
      redirect_to education_management_training_path(@training)
    else
      render :edit
    end
  end
